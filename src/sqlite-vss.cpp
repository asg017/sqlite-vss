...
static void vss_distance_l1(sqlite3_context * context, int argc, sqlite3_value ** argv) {
  ...
  if(a->size() != b->size()) {
    sqlite3_result_error(context, "vectors are not of the same size", -1);
    delete a;
    delete b;
    return;
  }
...
// Calculates Linf distance between two vectors.
static void vss_distance_linf(sqlite3_context * context, int argc, sqlite3_value ** argv) {
  ...
  if(a->size() != b->size()) {
    sqlite3_result_error(context, "vectors are not the same size", -1);
    delete a;
    delete b;
    return;
  }
...
std::vector<VssIndexColumn> * parse_constructor(int argc, const char *const*argv) {
  ...
  for(int i = 3; i < argc; i++) {
    ...
    auto name = arg.substr(0, lparen);
    auto sDimensions = arg.substr(lparen+1, rparen - lparen-1);
