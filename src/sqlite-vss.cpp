...
static void vss_distance_l1(sqlite3_context *context, int argc, sqlite3_value **argv) {
  ...
  if(a->size() != b->size()) {
    sqlite3_result_error(context, "LHS and RHS are vecors of different size", -1);
    return;
  }

  sqlite3_result_double(context, faiss::fvec_L1(a->data(), b->data(), a->size()));
}
...
static void vss_distance_l2(sqlite3_context *context, int argc, sqlite3_value **argv) {
  ...
  if(a->size() != b->size()) {
    sqlite3_result_error(context, "LHS and RHS are vectors of different size", -1);
    return;
  }

  sqlite3_result_double(context, faiss::fvec_L2sqr(a->data(), b->data(), a->size()));
}
...
static void vss_distance_linf(sqlite3_context *context, int argc, sqlite3_value **argv) {
  ...
  sqlite3_result_double(context, faiss::fvec_Linf(a->data(), b->data(), a->size()));
}
...
static void vss_inner_product(sqlite3_context *context, int argc, sqlite3_value **argv) {
  ...
  sqlite3_result_double(context, faiss::fvec_inner_product(a->data(), b->data(), a->size()));
}
...
static void vss_fvec_add(sqlite3_context *context, int argc, sqlite3_value **argv) {
  ...
  int d = a->size();
  auto c = ptr_vec(new std::vector<float>(d));
...
std::unique_ptr<std::vector<VssIndexColumn>> parse_constructor(int argc, const char * const * argv) {

  auto columns = std::unique_ptr<std::vector<VssIndexColumn>>(new std::vector<VssIndexColumn>());
  for(int i = 3; i < argc; i++) {
    ...
    std::string factory;
    if((factoryStart = arg.find("factory", rparen)) != std::string::npos && (factoryStringStartFrom = arg.find("=", factoryStart)) != std::string::npos) {
...
static int init(
  ...
  vss_index_vtab * pNew = (vss_index_vtab *) sqlite3_malloc(sizeof(* pNew));
  *ppVtab = (sqlite3_vtab *) pNew;
  ...
  for(int i = 0; i < pNew->indexCount; i++) {
    pNew->trainings.push_back(std::vector<float>());
    pNew->insert_to_add_data.push_back(std::vector<float>());
    pNew->insert_to_add_ids.push_back(std::vector<faiss::idx_t>());
    pNew->delete_to_delete_ids.push_back(std::vector<faiss::idx_t>());
  }
...
static int vssIndexFilter(
  ...
  if (strcmp(idxStr, "search") == 0) {
    ...
    ptr_vec query_vector;

    VssSearchParams* params;

    if ((params = (VssSearchParams*) sqlite3_value_pointer(argv[0], "vss0_searchparams")) != nullptr) {

      pCur->search_k = params->k;
      query_vector = ptr_vec(new std::vector<float>(*params->vector));
    ...
    } else if ((query_vector = pCur->table->vector_api->xValueAsVector(argv[0])) != nullptr) {
    ...
    sqlite3_stmt* stmt;
    int res = sqlite3_prepare_v2(pCur->table->db, sqlite3_mprintf("select rowid from \"%w_data\"", pCur->table->name), -1, &pCur->stmt, nullptr);
...
static int vssIndexColumn(
  ...
  if(i == VSS_INDEX_COLUMN_DISTANCE) {
    ...
    auto v = ptr_vec(new std::vector<float>(index->d));
    sqlite3_int64 rowid;
...
static int vssIndexUpdate(
  ...
  if (argc ==1 && sqlite3_value_type(argv[0]) != SQLITE_NULL) {
    ...
    if (noOperation) {
      ...
      for(int i = 0; i < p->indexCount; i++) {
        if ( (vec = p->vector_api->xValueAsVector(argv[2+VSS_INDEX_COLUMN_VECTORS + i])) != nullptr ) {
